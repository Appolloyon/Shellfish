...
for database in $DATAFILES; do
	...
	mkdir -p /home/bioinfor1/cklinger/"blastp_$db_short_name"

        ##send BLAST command to Cluster nodes
        blastp -query "$query" -db "$database" -out "blastp_$db_short_name/${q_short_name}_${db_short_name}.outfile.txt" -outfmt "6 qseqid sseqid evalue"  $BLAST_OPTIONS
    done
done
