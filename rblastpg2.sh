...
for database in $DATAFILES; do
	...
	for query in $QUERYFILES; do
        ...
        mkdir -p /home/cklinger/"rblastp_$q_short_name"
       
        
        ##send BLAST command to Cluster nodes
        blastp -query "$query" -db "$database" -out "rblastp_$q_short_name/${q_short_name}_${db_short_name}.outfile.txt" $BLAST_OPTIONS
    done
done
