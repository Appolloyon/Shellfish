...
QUERY_SUB_DIR=/home/bioinfor1/cklinger/BLAST_SEARCHES/TRblastpqueries
DATABASE_SUB_DIR=/home/bioinfor1/cklinger/BLAST_SEARCHES/TRblastpdatabase
...
for database in $DATAFILES; do
	...
	mkdir -p /home/bioinfor1/cklinger/"TRblastp_$db_short_name"

        ##send BLAST command to Cluster nodes
        blastp -query "$query" -db "$database" -out "TRblastp_$db_short_name/${q_short_name}_${db_short_name}.outfile.txt" -outfmt "6 qseqid sseqid evalue bitscore length"  $BLAST_OPTIONS
    done
done
