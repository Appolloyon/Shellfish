...
DATABASE_SUB_DIR=/home/cklinger/BLAST_SEARCHES/tblastnqueries
...
for database in $DATAFILES; do
	...
	mkdir -p /home/cklinger/"tblastn_$db_short_name"
        ...
        tblastn -query "$query" -db "$database" -out "tblastn_$db_short_name/${q_short_name}_${db_short_name}_${now}.outfile.txt" $BLAST_OPTIONS
    done
done
